...
const typeConfig = {
  ...
  idea_sent: {
    ...
    color: '#f59e0b',
    bg: '#fffbeb',
    description: (meta) => meta?.description || 'Sent an idea to a friend',
  },
  idea_received: {
    icon: 'fas fa-lightbulb',
    label: 'Idea Received',
    color: '#8b5cf6',
    bg: '#f5f3ff',
    description: (meta) => meta?.description || 'Received an idea from a friend',
  },
};
...
export default function RecentPage({ user }) {
  ...
  if (!user) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', padding: 24 }}>
        ...
        <h5 style={{ fontWeight: 700, color: '#1e293b', marginBottom: 8 }}>Sign in to view activity</h5>
  ...
  return (
    <div className="app-page-medium" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ color: '#64748b', fontSize: 14, margin: 0 }}>A complete history of everything you've done on Idea Pulse</p>
      </div>
      ...
      {!loading && activity.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 28 }}>
          {[
            { icon: 'fas fa-lightbulb', label: 'Ideas Generated', value: totalIdeas, color: '#46a29f', bg: '#e8f6f5' },
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 24, background: '#f8fafc', borderRadius: 10, padding: 4 }}>
          {[
            ...
            <button key={tab.key} onClick={() => setFilter(tab.key)} style={{
              ...
              color: filter === tab.key ? '#46a29f' : '#94a3b8',
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: '60px 0' }}>
          <div style={{ width: 36, height: 36, borderRadius: '50%', border: '3px solid #e2e8f0', borderTopColor: '#46a29f', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
