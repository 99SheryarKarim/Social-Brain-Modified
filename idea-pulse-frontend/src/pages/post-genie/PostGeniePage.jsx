...
const PostGeniePage = ({ user }) => {
    ...
    const getProviderFromModel = (modelId) => {
        ...
        if (modelId.includes('togethercomputer/') || modelId.includes('NousResearch/') || modelId.includes('Mistral-7B-Instruct-v0.2')) return 'together';
        if (modelId.includes('mistralai/') || modelId.includes('meta-llama/') || modelId.includes('NousResearch/')) return 'huggingface';
        return 'gemini';
    };

    const activeProvider = dataSource && ['gemini', 'huggingface', 'ollama', 'together'].includes(dataSource)
        ? dataSource
        : getProviderFromModel(selectedModel);

    const activeProviderLabel = providerLabels[activeProvider] || 'Gemini';
    const providerBadgeStyle = {
        backgroundColor: providerColors[activeProvider] || '#e8f6f5',
        color: activeProvider === 'gemini' ? '#1d4ed8' : activeProvider === 'huggingface' ? '#92400e' : activeProvider === 'together' ? '#166534' : '#5b21b6',
    };
