...
exports.sendOTPEmail = async (toEmail, otp) => {
  await getTransporter().sendMail({
    from: `"Idea Pulse" <${process.env.EMAIL_USER}>`,
    to: toEmail,
    subject: 'Your Idea Pulse Verification Code',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 480px; margin: auto; padding: 32px; border-radius: 12px; border: 1px solid #e0e0e0;">
        <h2 style="color: #667eea; margin-bottom: 8px;">⚡ Idea Pulse</h2>
  ...
  await getTransporter().sendMail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 520px; margin: auto; padding: 40px; border-radius: 12px; border: 1px solid #e0e0e0;">
        <h1 style="background: linear-gradient(135deg, #667eea, #764ba2); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-size: 28px;">⚡ Welcome to Idea Pulse!</h1>
        <p style="color: #444; font-size: 15px; line-height: 1.6;">
          Hi <strong>${toEmail}</strong>,<br/><br/>
          Your account has been successfully created. You're now part of the Idea Pulse community!
        </p>
        ...
        <p style="color: #aaa; font-size: 12px;">© 2025 Idea Pulse. All rights reserved.</p>
      </div>
    `,
  });
};
