  ...
  try {
    ...
    db.run(
      ...
      (err) => {
        ...
        logActivity(userId, 'facebook_connected', `Connected Facebook page "${pageName || pageId}"`, { pageId, pageName });
...
exports.publishPost = async (req, res) => {
  ...
  db.get(`SELECT facebook_token FROM users WHERE id = ?`, [userId], async (err, row) => {
    ...
    try {
      ...
      const topic = req.body.originalTopic || content.slice(0, 50);
      await logActivity(userId, 'post_uploaded', `Published "${topic}" to Facebook`, { topic, facebookPostId: fullPostId });
